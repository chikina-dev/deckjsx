...
export function deckjsxRuntimeDataUrl(specifier: string, key: string): string | undefined {
...
export function nodeRuntimeDataUrl(key: string): string {
  return executableDataUrl(
    [
...
export function executableDataUrl(code: string, serial: number): string {
  return `data:text/javascript;base64,${Buffer.from(code).toString("base64")}#deckjsx-dev-${process.pid}-${Date.now()}-${serial}`;
}

export async function awaitDefaultExport(defaultExport: unknown): Promise<void> {
...
export function resolveDeckjsxExternalImports(
  code: string,
...
export function captureDefaultExport(code: string, key: string): string {
  const program = parseAst(code, { lang: "js", sourceType: "module" }, "deckjsx-dev-entry.mjs");
...
export function resolveDeckjsxPackageImport(specifier: string, cwd: string): string {
